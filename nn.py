...
your_feature_spec = {
	"number": tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, 1], name='number'),
...
def serving_input_receiver_fn():
	...
	"""
	mu_selected = tf.compat.v1.placeholder(dtype=tf.double, shape=[None, 2], name='mu_selected')
	mu_left = tf.compat.v1.placeholder(dtype=tf.double, shape=[None, 2], name='mu_left')
	mu_v=tf.compat.v1.placeholder(dtype=tf.double, shape=[None, 2], name='mu_v')
...
def init(learningRate1, numOfEpochs1, batchSize1, dimension1, model_iter1):
	...
	model = tf.estimator.Estimator(neuralNetModel,model_dir = model_dir_name+"temp", config=run_config)
...
def neuralNetModel(features, labels, mode):
	...
	layer_1 = tf.compat.v1.layers.dense(features[0],dimension,activation=None)

	# layer 2 for the left nodes
	layer_2 = tf.compat.v1.layers.dense(features[1],dimension,activation=None)

	# layer 3 for the selected nodes
	layer_3 = tf.compat.v1.layers.dense(features[2],dimension,activation=None)
...
#	layer_5 = tf.nn.leaky_relu(layer_4)
	...
	output = tf.compat.v1.layers.dense(layer_4,1,activation=None)
	...
	loss_op = tf.reduce_mean(input_tensor=tf.math.squared_difference(output, labels))
	...
	train_op = optimizer.minimize(loss_op, global_step=tf.compat.v1.train.get_global_step())
...
def trainNeuralNet(historyOfTuples):
	...
	#	print(" y train ",y_train)
		...
		input_fn = tf.compat.v1.estimator.inputs.numpy_input_fn(x={'mu_selected': mu_s, 'mu_left': mu_l, 'mu_v': mu_v}, y=y_train, batch_size=3, num_epochs=500, shuffle=False)
...
def getBestNode(graphid, stateIdx):
	...
	input_fn = tf.compat.v1.estimator.inputs.numpy_input_fn(x={'mu_selected': mu_s, 'mu_left': mu_l, 'mu_v': mu_v}, shuffle=False)
