...
class NodeMinibatchIterator(object):
    ...
    def __init__(self, G, id2idx, 
        ...
        if (mode =="train"):
            ...
            self.train_nodes = [n for n in self.train_nodes if not set(self.G.neighbors(n)).intersection(self.no_train_nodes_set)]
    ...
    def construct_adj(self):
        ...
        for nodeid in self.G.nodes():
            ...
            #neighbors = []
            #for neighbor in self.G.neighbors(nodeid):
            #    print('nodeid', nodeid, 'neighbor',neighbor)
            #    if(str(neighbor) in self.id2idx.keys()):
            #        print('sss')
            #        neighbors = neighbors + np.array(self.id2idx[str(neighbor)])
            neighbors = np.array([self.id2idx[str(neighbor)] 
                for neighbor in self.G.neighbors(nodeid)
               ])
            deg[self.id2idx[str(nodeid)]] = len(neighbors)
    ...
    def construct_adj_and_feat_for_top_degree_testing(self, budget=None, bud_mul_fac=None,neighborhood_sampling=None):
        ...
        top_nodes = [node for node in self.G.nodes() if self.G.nodes[node]['bipartite']==0]
        bottom_nodes = [node for node in self.G.nodes() if self.G.nodes[node]['bipartite']==1]
   ...
   #     deg=np.zeros((len(self.id2idx),))
      ...
      #  print("len degree_of_nodes_left ", len(degree_of_nodes_left))
        degree_of_nodes_all=[degree for node, degree in degree_of_nodes_all]
      ...
      #  list_top_nodes_and_sampled_neighbors=list()#np.array([])#set()
       ...
       # np_array_neighbors = {}
        ...
        #    list_top_nodes_and_sampled_neighbors.append(dict_map_real_to_counter_node[nodeid])
         ...
         #   print(list_of_all_neighbors)
            ...
            deg[dict_map_real_to_counter_node[nodeid]]=len(list(neighbors))
       ...
       #     neighbors_weights=np.array([self.G[nodeid][neighbor]['weight'] for neighbor in neighbors])
        #    neighbors_weights=neighbors_weights / sum(neighbors_weights)
            ...
            if len(list(neighbors)) > self.max_degree:
                neighbors=np.random.choice(neighbors, self.max_degree,replace=False)
            elif len(list(neighbors)) < self.max_degree:
                neighbors=np.random.choice(neighbors, self.max_degree, replace=True)

            # time_A=time.time() - time_sampling_beg
            # print(" TIME A ", time_A)
          #  adj[self.id2idx[str(nodeid)], :]=np.asarray(list(neighbors))
           # adj[self.id2idx[str(nodeid)]]
